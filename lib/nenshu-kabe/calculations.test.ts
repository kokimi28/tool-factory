...
describe("記事 nenshu-kabe-haigusha-kojo（C11）の配偶者本人手取りの二重化", () => {
  // 記事の配偶者本人手取り（給与123万→手取り121万＝税の壁は逆転しない）を固定（品質ゲート①）。
  // 配偶者控除・配偶者特別控除そのものは lib/nenshu-kabe/spouse-deduction.ts でモデル化済み
  // （G6。以前は「法定値で calc 未モデルのため本文で明記」としていたが、その結果
  //  103万・150万・201.6万という改正前の値が本文に残り続けていた）。
  it("配偶者 給与123万→本人手取り1,210,000（税の壁は逆転しない）", () => {
    expect(takeHomeAtIncome(1_230_000, false).takeHome).toBe(1_210_000);
  });
});
