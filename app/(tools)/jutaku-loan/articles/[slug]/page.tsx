...
import { ARTICLES, getArticle } from '@/lib/jutaku-loan/articles';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/jutaku-loan/articles/${article.slug}` },
    openGraph: {
      title: article.title,
      description: article.description,
      type: 'article',
        ...
        <article className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-lg p-6 space-y-8 text-sm text-gray-800 leading-relaxed">
          ...
          <section className="rounded-lg bg-blue-50 border border-blue-100 p-4 space-y-2">
            <p className="text-gray-800">
              各年の控除額と総額は、
              <Link href="/jutaku-loan" className="text-blue-600 hover:underline font-medium">
                住宅ローン控除シミュレーター
              </Link>
              に借入額・金利・住宅性能を入力すればすぐに確認できます。
            </p>
            <p className="text-gray-800">
              関連ツール：
