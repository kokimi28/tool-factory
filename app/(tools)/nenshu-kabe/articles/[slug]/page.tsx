...
import { ARTICLES, getArticle } from '@/lib/nenshu-kabe/articles';
import { SITE_META } from '@/lib/nenshu-kabe/site-meta';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/nenshu-kabe/articles/${article.slug}` },
    openGraph: {
      title: article.title,
      description: article.description,
      type: 'article',
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 py-6">
          <Link href="/nenshu-kabe" className="text-sm text-blue-600 hover:underline">
            ← 年収の壁 手取り逆転シミュレーターに戻る
          </Link>
        ...
        <article className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-lg p-6 space-y-8 text-sm text-gray-800 leading-relaxed">
          ...
          <section className="rounded-lg bg-blue-50 border border-blue-100 p-4 space-y-2">
            <p className="text-gray-800">
              あなたの年収での手取りの逆転・回復ラインは、
              <Link href="/nenshu-kabe" className="text-blue-600 hover:underline font-medium">
                年収の壁 手取り逆転シミュレーター
              </Link>
              で確認できます。
            </p>
            <p className="text-gray-800">
              ...
              ／
              <Link href="/furusato" className="text-blue-600 hover:underline">
                ふるさと納税 限度額
              </Link>
            </p>
          </section>
