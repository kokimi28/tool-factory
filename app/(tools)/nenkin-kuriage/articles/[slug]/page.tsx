...
import { ARTICLES, getArticle } from '@/lib/nenkin-kuriage/articles';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/nenkin-kuriage/articles/${article.slug}` },
    openGraph: {
      title: article.title,
      description: article.description,
      type: 'article',
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <p className="text-xs text-gray-500 mt-2">
            ...
            {SITE_META.appliedLawDate}の制度に基づく）
          </p>
        </div>
      </header>
        ...
        <article className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-lg p-6 space-y-8 text-sm text-gray-800 leading-relaxed">
          ...
          <section className="rounded-lg bg-blue-50 border border-blue-100 p-4 space-y-2">
            <p className="text-gray-800">
              開始年齢別の月額・累計・損益分岐年齢は、
              <Link href="/nenkin-kuriage" className="text-blue-600 hover:underline font-medium">
            ...
            <p className="text-gray-800">
              関連ツール：
              <Link href="/ideco" className="text-blue-600 hover:underline">
                iDeCo受取税シミュレーター
              </Link>
              ／
              <Link href="/taishokukin" className="text-blue-600 hover:underline">
                退職金の税金
              </Link>
            </p>
          </section>
