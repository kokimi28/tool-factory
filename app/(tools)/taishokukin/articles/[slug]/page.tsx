...
import { ARTICLES, getArticle } from '@/lib/taishokukin/articles';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/taishokukin/articles/${article.slug}` },
    openGraph: {
      title: article.title,
      description: article.description,
      type: 'article',
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <p className="text-xs text-gray-500 mt-2">
            ...
            {SITE_META.appliedLawDate}現在の法令に基づく）
          </p>
        </div>
      </header>
        ...
        <article className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-lg p-6 space-y-8 text-sm text-gray-800 leading-relaxed">
          ...
          <section className="rounded-lg bg-blue-50 border border-blue-100 p-4">
            <p className="text-gray-800">
              実際の控除額・税額・手取りは、
              <Link href="/taishokukin" className="text-blue-600 hover:underline font-medium">
                退職金課税シミュレーター
              </Link>
              に金額と勤続年数を入力すればすぐに確認できます。「あと1年勤めると」の差額も表示します。
            </p>
          </section>
